...
Vec2[] acc = new Vec2[maxNumAgents];
float[] origDist = new float[maxNumAgents];
...
void setup(){
  ...
  agentPos[3] = new Vec2(600,520);
  agentPos[4] = new Vec2(480,400);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(350,200);
  goalPos[4] = new Vec2(540,120);
  acc[0] = new Vec2(0,0);
  acc[1] = new Vec2(0,0);
  acc[2] = new Vec2(0,0);
  ...
  for (int i = 0; i < numAgents; i++){
    ...
    if (agentVel[i].length() > 0) {
     ...
     origDist[i] = goalPos[i].minus(agentPos[i]).length();
  }
}
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  ...
  Vec2 v = vel2.minus(vel1);
  float a = dot(v, v);
  float b = dot(w, v)*2;
  float c = dot(w, w)-(radius1+radius2)*(radius1+radius2);
  float time1 = (-b + sqrt(b*b-4*a*c))/(2*a);
  float time2 = (-b - sqrt(b*b-4*a*c))/(2*a);
  if(time1 < 0 && time2 < 0) {
    return -1;
  }
...
Vec2 computeAgentForces(Vec2 force, int i){
...
Vec2[] goalVel = new Vec2[maxNumAgents];


//Update agent positions & velocities based acceleration
void moveAgent(float dt){
  for(int i = 0; i < numAgents; i++) {
    goalVel[i] = (goalPos[i].minus(agentPos[i])).normalized().times(200);
  }
  //Compute accelerations for every agents
  for (int i = 0; i < numAgents; i++){
    Vec2 goal_force = goalVel[i].minus(agentVel[i]);
    //k_goal = 1/origDist[i]*goalPos[i].minus(agentPos[i]).length();
    //if(goalPos[i].minus(agentPos[i]).length() < 50) {
    //  float tmp = 
    //}
    //else {
    ...
    //}
    goal_force.mul(k_goal);
    agentAcc[i] = computeAgentForces(goal_force, i);
    int index = 0;
    while(index < maxNumAgents) {
      if(index != i && agentPos[i].minus(goalPos[i]).length() > 100) {
        float time = computeTTC(agentPos[i], agentVel[i], agentRad, agentPos[index], agentVel[index], agentRad);
        if(time > 0) {
          Vec2 A_future = agentPos[i].plus(agentVel[i].times(time));
          Vec2 B_future = agentPos[index].plus(agentVel[index].times(time));
          Vec2 relative_future_direction = A_future.minus(B_future).normalized();
          agentAcc[i].add(relative_future_direction.times(k_avoid*(1/time)));
          println(agentAcc[i]);
        }
      }
      index++;
